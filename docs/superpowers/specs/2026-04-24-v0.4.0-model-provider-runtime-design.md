...
`v0.4.0` is an inserted release between Phase 2 and Phase 3 of the runtime architecture roadmap.

It does not replace Phase 3, Phase 4, or Phase 5:

- Phase 3 remains focused on session workflow assets such as checkpoint, fork, compact, and handoff.
- Phase 4 remains focused on headless runtime interfaces such as JSONL/RPC.
- Phase 5 remains focused on observability and governance such as cost tracking, audit, export, and debug/replay.

This release prepares those phases by strengthening two RFC-level runtime boundaries that already existed in the original architecture:

- `model`: provider client and completion interface
- `runtime`: turn runner, event flow, and model/tool lifecycle

The release is therefore directionally aligned with the original roadmap: it moves provider choice and streaming out of the CLI surface and into reusable runtime infrastructure.
...
cliq --provider anthropic --model claude-sonnet-4-20250514 "inspect the repo"
cliq --provider openai --model gpt-5.2 "explain this file"
...
### 8.2 Why Static Capabilities Instead Of Dynamic Discovery
...
Runtime events complement existing runtime hooks. Hooks remain the extension-side lifecycle mechanism; runtime events become the normalized surface/reporting mechanism for CLI output, future JSONL/RPC, and later observability.

### 9.6 Session Schema Update
...
- CLI arg parsing for `--provider`, `--model`, `--base-url`, and `--streaming`
- workspace config parsing and validation for `model`, including streaming mode validation
